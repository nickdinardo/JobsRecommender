...
def read_dataset(path,
    ...
    if not ('item_idx' in data.columns and 'user_idx' in data.columns):
        ...
        idx_to_user = pd.Series(index=user_to_idx.data, data=user_to_idx.index.values)
        #  map ids to indices
...
def df_to_csr(df, nrows, ncols, is_implicit=False, user_key='user_idx', item_key='item_idx', rating_key='interaction_type'):
    """
...
# dome utility functions to save and load sparse matrices
...
def load_sparse(filename):
    ...
    return sps.csc_matrix((loader['data'], loader['indices'], loader['indptr']), shape=loader['shape'])
